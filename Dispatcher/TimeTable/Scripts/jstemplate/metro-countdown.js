    ...
    $.widget("metro.countdown", 
        ...
        tick: function()
            ...
            this._interval = setInterval(function()
                ...
                // Number of minutes lef
                m = Math.floor(left / minutes)
                that.updateDuo(4, 5, m)
                left -= m*minutes

                // Number of seconds lef
                s = left
                that.updateDuo(6, 7, s)

                // Calling an optional user supplied ontic
                that.options.ontick(d, h, m, s)

                that.blinkDivider()

                // Scheduling another call of this function in 1
                if (d === 0 && h === 0 && m === 0 && s === 0) 
                    that.options.onstop()
                    that.stopDigit()
                    that._trigger('alarm')
        ...
        switchDigit: function(wrapper, number)
            ...
            if(wrapper.data('digit') == number)
                // We are already showing this numbe
                return false
